...
/**
 * DAO class to check if a book is already in a wishlist.
 */
    ...
    protected void doAccess() throws Exception {
        try (PreparedStatement stmnt = con.prepareStatement(IS_BOOK_IN_WISHLIST)) {
            ...
            try (ResultSet rs = stmnt.executeQuery()) {
                ...
                LOGGER.info("Book ID {} is {}in wishlist ID {}.", book.getBookId(), this.outputParam ? "" : "not ", wishlist.getWishlistId());
            }

        } catch (Exception ex) {
            LOGGER.error("Error checking if book ID {} is in wishlist ID {}: {}", book.getBookId(), wishlist.getWishlistId(), ex.getMessage());
        }
    }
}