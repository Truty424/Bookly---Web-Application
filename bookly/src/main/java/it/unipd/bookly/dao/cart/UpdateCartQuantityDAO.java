...
public class UpdateCartQuantityDAO extends AbstractDAO<Void> {
    ...
    protected void doAccess() throws Exception {
        try (PreparedStatement stmnt = con.prepareStatement(UPDATE_CART_QUANTITY)) {
            ...
            if (rows > 0) {
                LOGGER.info("Cart {} updated with quantity {}.", cartId, quantity);
            } else {
                LOGGER.warn("No cart found with ID {} to update quantity.", cartId);
            }

        } catch (Exception ex) {
            LOGGER.error("Error updating quantity for cart {}: {}", cartId, ex.getMessage());
            throw ex;
        }
    }
}
