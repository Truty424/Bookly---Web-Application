...
public class AddPublisherToBookDAO extends AbstractDAO<Boolean> {
     ...
     */
    public AddPublisherToBookDAO(final Connection con, final int bookId, final int publisherId) {
        super(con);
        this.bookId = bookId;
    ...
    protected void doAccess() throws SQLException {
        try (PreparedStatement stmt = con.prepareStatement(ADD_PUBLISHER_TO_BOOK)) {
            ...
            if (outputParam) {
                LOGGER.info("Publisher {} successfully assigned to book {}.", publisherId, bookId);
            } else {
                LOGGER.warn("No rows affected when assigning publisher {} to book {}.", publisherId, bookId);
            }

        } catch (SQLException ex) {
            LOGGER.error("Error assigning publisher {} to book {}: {}", publisherId, bookId, ex.getMessage());
            throw ex;
        }
    }
}
